#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Contains different layers (keras tf.keras.layers.Layer classes) for keras neural network.

List of classes:

    * Non_local_block_multi_head - The multi-head convolutional self-attention layer with positional encoding (if needed).
    * _Self_attention_non_local_block_without_shortcut_connection - The self-attention layer with positional encoding (if needed)
      It is used only for inner purposes, because it does not have shortcut connection, which makes it not as described in paper.
...
class Non_local_block_multi_head(tf.keras.layers.Layer):
    """ The multi-head self-attention layer, which is based on the convolutions and relative positional encoding (if chosen).
        The paper describes this attention is:
        https://openaccess.thecvf.com/content_cvpr_2018/papers/Wang_Non-Local_Neural_Networks_CVPR_2018_paper.pdf

        The multi-head attention is realised by myself.
        """
    ...
    def build(self, input_shape):
        # construct heads
        self.heads = [
            _Self_attention_non_local_block_without_shortcut_connection(output_channels=self.head_output_channels,
                                                                        downsize_factor=self.downsize_factor,
                                                                        conv_type='2D', name_prefix="head_%i" % _,
        ...
        with tf.name_scope(name="multi_head_attention_output_conv"):
            self.output_conv.build(
                (input_shape[0], input_shape[1], input_shape[2], self.head_output_channels * len(self.heads)))
...
class _Self_attention_non_local_block_without_shortcut_connection(tf.keras.layers.Layer):
    """ The self-attention layer, which is based on the convolutions and relative positional encoding (if chosen).
    This class differs from others by deleting the shortcut connection to the end of the attention.
    The paper describes this attention is:
    https://openaccess.thecvf.com/content_cvpr_2018/papers/Wang_Non-Local_Neural_Networks_CVPR_2018_paper.pdf

    """

    def __init__(self, output_channels: int, downsize_factor: int = 1, conv_type: str = 'spatio-temporal',
        ...
        :param kwargs:
        """
        if conv_type not in ('1D', '2D', '3D'):
            raise AttributeError('Mode can be either \'spatial\' or \'spatio-temporal\'. Got %s.' % conv_type)
        ...
        if self.relative_pos_enc:
            if self.conv_type != '2D':
                raise AttributeError('Relative positional encoding works only with \'2D\' mode.')
    ...
    def call(self, input):
        ...
        output_key_conv = tf.keras.layers.Reshape((-1, tf.keras.backend.int_shape(x)[-1]),
                                                  name=self.name_prefix + "_reshape_key_conv")(x)
        # query flow
        x = self.query_conv(input)
        output_query_conv = tf.keras.layers.Reshape((-1, tf.keras.backend.int_shape(x)[-1]),
                                                    name=self.name_prefix + "_reshape_query_conv")(x)
        # value flow
        x = self.value_conv(input)
        output_value_conv = tf.keras.layers.Reshape((-1, tf.keras.backend.int_shape(x)[-1]),
                                                    name=self.name_prefix + "_reshape_value_conv")(x)
        # matrix multiplication for query and key
        multiplicated_key_query = tf.keras.layers.Dot(axes=2, name=self.name_prefix + "dot_key_query")(
            [output_key_conv, output_query_conv])
        ...
        output_value = tf.keras.layers.Dot(axes=(2, 1), name=self.name_prefix + "dot_key_query_value")(
            [softmax_output, output_value_conv])
        # reshape
...
class Self_attention_non_local_block(tf.keras.layers.Layer):
    """ The self-attention layer, which is based on the convolutions and relative positional encoding (if chosen).
    This class represents the layer descrived in the paper:
    https://openaccess.thecvf.com/content_cvpr_2018/papers/Wang_Non-Local_Neural_Networks_CVPR_2018_paper.pdf
    """

    def __init__(self, output_channels: int, downsize_factor: int = 1,
                 conv_type: str = 'spatio-temporal', name_prefix: str = "attention",
        ...
        if conv_type not in ('2D', '3D'):
            raise AttributeError('Mode can be either \'2D\' or \'3D\'. Got %s.' % conv_type)
        self.conv_type = conv_type
        self.relative_positional_encoding = relative_position_encoding
        self.downsize_factor = downsize_factor
        self.output_channels = output_channels
    ...
    def build(self, input_shape):
        ...
        self.non_local_block_without_shortcut = _Self_attention_non_local_block_without_shortcut_connection(
